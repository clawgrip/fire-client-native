...
public final class TestProviderAOSigner {
	...
	public static void main(final String[] args) throws Exception {
		...
		final AOSigner pdfSigner = new ClaveFirmaPdfSigner();
		...
		try (
			final OutputStream fos = new FileOutputStream(File.createTempFile("CLAVE_FULL", ".pdf")); //$NON-NLS-1$ //$NON-NLS-2$
		) {
			fos.write(pdf);
			fos.close();
		}
